...
import logging
import torch
import torch.nn.functional as F  # newly added
        ...
        for dataset in self.datasets:
            ...
            for ii in range(0, len(gs_scores), params.batch_size):
                ...
                if len(batch1) == len(batch2) and len(batch1) > 0:
                    ...
                    enc1_pos = F.normalize(enc1_pos, p=2, dim=1)
                    enc2_pos = F.normalize(enc2_pos, p=2, dim=1)
                    loss_align = self.align_loss(enc1_pos, enc2_pos)
                    loss_uniform = self.uniform_loss(torch.cat((enc1_pos, enc2_pos), dim=0))
